    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-navy/50">
          <div>
            <h2 className="font-agency text-lg text-white uppercase tracking-wider">
              Carrito
            </h2>
            <p className="text-xs text-ash">{totalItems} artículo{totalItems !== 1 ? 's' : ''}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-12">
              ...
              <p className="font-agency text-ash uppercase tracking-wider text-sm">
                Tu carrito está vacío
              </p>
              <button
                onClick={onClose}
                className="mt-4 text-xs text-dragon hover:text-frost transition-colors"
              >
                Explorar catálogo →
              </button>
            </div>
